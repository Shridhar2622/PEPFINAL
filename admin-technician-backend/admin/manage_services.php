...
if (!empty($search)) {
    $search_param = "%$search%";
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM services WHERE product_name LIKE ? OR problem_description LIKE ?");
  ...
  WHERE services.product_name LIKE ? OR services.problem_description LIKE ? 
...
");
    ...
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM services")->fetch_assoc()['total'];
    $result = $conn->query("
  SELECT services.*, categories.name AS category_name 
  FROM services 
  LEFT JOIN categories ON services.category_id = categories.id 
  ...
  <div class="container">
    <div class="sidebar">
        ...
        <div class="submenu" id="manageMenu">
            <a href="manage_services.php" class="active"><i class="fas fa-tools"></i> Manage Services</a>
          ...
          <?php if ($result && $result->num_rows > 0): ?>
            <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td>
                  <a href="edit_service.php?id=<?= $row['id'] ?>" title="Edit"><i class="fas fa-edit"></i></a>
                  &nbsp;
                  <a href="delete_service.php?id=<?= $row['id'] ?>" title="Delete" onclick="return confirm('Are you sure you want to delete this service?')"><i class="fas fa-trash-alt"></i></a>
                </td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
