...
if (!empty($search)) {
    $search_param = $search;
    // Using FULLTEXT search on amount_reasons.reason
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM amount_reasons WHERE MATCH(id, reason) AGAINST(? IN BOOLEAN MODE)");
    $stmt_count->bind_param("s", $search_param);
    $stmt_count->execute();
    $total_records = $stmt_count->get_result()->fetch_assoc()['total'];

    $stmt = $conn->prepare("SELECT * FROM amount_reasons WHERE MATCH(id, reason) AGAINST(? IN BOOLEAN MODE) ORDER BY id DESC LIMIT ? OFFSET ?");
    $stmt->bind_param("sii", $search_param, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM amount_reasons")->fetch_assoc()['total'];
    $result = $conn->query("SELECT * FROM amount_reasons ORDER BY id DESC LIMIT $limit OFFSET $offset");
}
  ...
  <div class="container">
    <div class="sidebar">
        ...
        <div class="submenu" id="manageMenu">
            ...
            <a href="manage_reason.php" class="active"><i class="fas fa-receipt"></i> Manage Reason</a>
        </div>
    ...
    <main class="main">
      ...
      <form method="GET" class="search-form" action="">
        <input type="text" name="search" placeholder="Search reasons" value="<?= htmlspecialchars($search) ?>" />
            ...
            <?php if ($result && $result->num_rows > 0): ?>
              <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
                <tr>
                  ...
                  <td>
                    <a href="edit_reason.php?id=<?= $row['id'] ?>" title="Edit"><i class="fas fa-edit"></i></a>
                    &nbsp;
                    <a href="delete_reason.php?id=<?= $row['id'] ?>" title="Delete" onclick="return confirm('Are you sure you want to delete this reason?')"><i class="fas fa-trash-alt"></i></a>
                  </td>
                </tr>
              <?php endwhile; ?>
            <?php else: ?>
              <tr>
                <td colspan="3" class="no-data">No reasons found.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
