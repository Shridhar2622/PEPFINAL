...
if (!empty($search)) {
    ...
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total 
        ...
        WHERE 
            ...
            phone LIKE ? OR 
            address LIKE ? OR 
            customer_note LIKE ? OR 
            technician_note LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT 
                                pre_transport_orders.*, 
                                amount_reasons.reason AS extra_reason
                            FROM pre_transport_orders 
                            LEFT JOIN amount_reasons ON pre_transport_orders.extra_reason_id = amount_reasons.id
                            WHERE 
                                pre_transport_orders.product_name LIKE ? OR 
                                pre_transport_orders.problem_description LIKE ? OR 
                                pre_transport_orders.customer_name LIKE ? OR 
                                pre_transport_orders.phone LIKE ? OR 
                                pre_transport_orders.address LIKE ? OR 
                                pre_transport_orders.customer_note LIKE ? OR 
                                pre_transport_orders.technician_note LIKE ?
                            ORDER BY pre_transport_orders.id DESC 
                            LIMIT ? OFFSET ?");
    ...
    // No search term
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM pre_transport_orders")->fetch_assoc()['total'];
    $result = $conn->query("
        SELECT 
            pre_transport_orders.*, 
            ...
            amount_reasons.reason AS extra_reason
        FROM pre_transport_orders
        JOIN dealer ON pre_transport_orders.dealer_id = dealer.id
        LEFT JOIN amount_reasons ON pre_transport_orders.extra_reason_id = amount_reasons.id
        ORDER BY pre_transport_orders.id DESC 
...
<div class="container">
    <div class="sidebar">
        ...
        <a href="pre_transport_orders.php" class="active"><i class="fas fa-clipboard-list"></i> Pre Transport Orders</a>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if ($row['status'] == 'assigned' || $row['status'] == 'done') {
                                        ...
                                        ?>
                                        <form method="POST" action="pre_transport_assign_order.php" class="assign-form">
                                ...
                                <td>
                                    <?php if ($row['status'] === 'pending'): ?>
                                        <form method="POST" action="pre_transport_cancel_order.php" onsubmit="return confirm('Are you sure you want to cancel this order?');">
                                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>" />
                                            <button type="submit" class="cancel-btn">Cancel</button>
                                        </form>
                                    <?php elseif ($row['status'] === 'assigned'): ?>
                                        <form method="POST" action="pre_transport_remove_assignment.php" onsubmit="return confirm('Remove technician assignment and set back to pending?');">
                                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>" />
