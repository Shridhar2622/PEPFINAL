...
function generateUniqueIdNumber($conn) {
    do {
        ...
        $stmt = $conn->prepare("SELECT id FROM techniciam WHERE id_number = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upload_dir = '../uploads/technician_id_proof/';
    $id_proof_path = '';
    $allowed_img_types = ['jpg', 'jpeg', 'png'];
    $allowed_pdf = ['pdf'];
    
    if (isset($_FILES['id_proof']) && count($_FILES['id_proof']['name']) > 0) {
        $file_count = count($_FILES['id_proof']['name']);
    
        if ($file_count > 2) {
            $errors['id_proof'] = "You can upload a maximum of 2 files.";
        } else {
            $files = $_FILES['id_proof'];
            $exts = [];
    
            for ($i = 0; $i < $file_count; $i++) {
                $exts[] = strtolower(pathinfo($files['name'][$i], PATHINFO_EXTENSION));
            }
    
            // Check if all are images
            $all_images = !array_diff($exts, $allowed_img_types);
            $all_pdf = !array_diff($exts, $allowed_pdf);
    
            if ($all_pdf) {
                // Just save the first one (if two, ignore second)
                $filename = uniqid('id_', true) . '.pdf';
                $destination = $upload_dir . $filename;
    
                if (move_uploaded_file($files['tmp_name'][0], $destination)) {
                    $id_proof_path = $destination;
                } else {
                    $errors['id_proof'] = "Failed to upload PDF.";
                }
                ...
                if ($file_count == 1) {
                    ...
                    if (move_uploaded_file($files['tmp_name'][0], $destination)) {
                        ...
                        $errors['id_proof'] = "Failed to upload image.";
                    }
                } elseif ($file_count == 2) {
                    // Combine two images
                    $img1 = ($exts[0] === 'png') ? imagecreatefrompng($files['tmp_name'][0]) : imagecreatefromjpeg($files['tmp_name'][0]);
                    $img2 = ($exts[1] === 'png') ? imagecreatefrompng($files['tmp_name'][1]) : imagecreatefromjpeg($files['tmp_name'][1]);
                    ...
                    if ($img1 && $img2) {
                        ...
                        imagecopy($combined, $img1, 0, 0, 0, 0, $width1, $height1);
                        imagecopy($combined, $img2, $width1, 0, 0, 0, $width2, $height2);
    
                        $filename = uniqid('id_combined_', true) . '.jpg';
                        $destination = $upload_dir . $filename;
    
                        if (imagejpeg($combined, $destination)) {
                            $id_proof_path = $destination;
                        } else {
                            $errors['id_proof'] = "Failed to save combined image.";
                        }
                        ...
                        $errors['id_proof'] = "Invalid image files.";
                    }
                }
            } else {
                $errors['id_proof'] = "Only JPG, PNG or PDF files are allowed.";
            }
        }
    } else {
        $errors['id_proof'] = "Please upload ID proof.";
    }
    
    $agreement_dir = '../uploads/technician_agreement/';
    $agreement_path = '';
    $allowed_img_types = ['jpg', 'jpeg', 'png'];
    $allowed_pdf = ['pdf'];
    
    if (isset($_FILES['agreement']) && count($_FILES['agreement']['name']) > 0) {
        $file_count = count($_FILES['agreement']['name']);
    
        if ($file_count > 2) {
            $errors['agreement'] = "You can upload a maximum of 2 agreement files.";
        } else {
            $files = $_FILES['agreement'];
            $exts = [];
            ...
            if ($all_pdf) {
                $filename = uniqid('agr_', true) . '.pdf';
                $destination = $agreement_dir . $filename;
    
                if (move_uploaded_file($files['tmp_name'][0], $destination)) {
                    $agreement_path = $destination;
                } else {
                    $errors['agreement'] = "Failed to upload agreement PDF.";
                }
    
            } elseif ($all_images) {
                if ($file_count == 1) {
                    $ext = $exts[0];
                    $filename = uniqid('agr_', true) . '.' . $ext;
                    $destination = $agreement_dir . $filename;
    
                    if (move_uploaded_file($files['tmp_name'][0], $destination)) {
                        $agreement_path = $destination;
                    } else {
                        $errors['agreement'] = "Failed to upload agreement image.";
                    }
                } elseif ($file_count == 2) {
                    $img1 = ($exts[0] === 'png') ? imagecreatefrompng($files['tmp_name'][0]) : imagecreatefromjpeg($files['tmp_name'][0]);
                    $img2 = ($exts[1] === 'png') ? imagecreatefrompng($files['tmp_name'][1]) : imagecreatefromjpeg($files['tmp_name'][1]);
                    ...
                    if ($img1 && $img2) {
                        ...
                        $filename = uniqid('agr_combined_', true) . '.jpg';
                        $destination = $agreement_dir . $filename;
    
                        if (imagejpeg($combined, $destination)) {
                            $agreement_path = $destination;
                        } else {
                            $errors['agreement'] = "Failed to save combined agreement image.";
                        }
                        ...
                        $errors['agreement'] = "Invalid agreement image files.";
                    }
                }
            } else {
                $errors['agreement'] = "Only JPG, PNG or PDF files are allowed for agreement.";
            }
        }
    } else {
        $errors['agreement'] = "Please upload the agreement.";
    }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO techniciam (name, email, phone, password, address, id_proof, agreement, id_number, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssssssss", $name, $email, $phone, $hashed_password, $address, $id_proof_path, $agreement_path, $id_number);
  ...
  <div class="container">
    <div class="sidebar">
        ...
        <div class="submenu" id="addMenu">
            ...
            <a href="add_technician.php" class="active"><i class="fas fa-user-plus"></i> Add Technician</a>
    ...
    <main>
      ...
      <form method="POST" action="" enctype="multipart/form-data">
        ...
        <label for="id_proof">Upload ID Proof (JPG, PNG, PDF – Max 2 files)</label>
        <input type="file" name="id_proof[]" accept=".jpg, .jpeg, .png, .pdf" multiple />
        <?php if (!empty($errors['id_proof'])): ?>
          <div class="error"><?= htmlspecialchars($errors['id_proof']) ?></div>
        <?php endif; ?>
        
        <label for="agreement">Upload Agreement (JPG, PNG, PDF – Max 2 files)</label>
        <input type="file" name="agreement[]" accept=".jpg, .jpeg, .png, .pdf" multiple />
