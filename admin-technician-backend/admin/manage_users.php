...
if (!empty($search)) {
    $search_param = "%$search%";
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM users WHERE name LIKE ? OR email LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT * FROM users WHERE name LIKE ? OR email LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?");
              ...
              <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
                <tr>
                  ...
                  <td>
                    ...
                    &nbsp;
                    <a href="delete_user.php?id=<?= $row['id'] ?>" title="Delete" onclick="return confirm('Are you sure you want to delete this user?')"><i class="fas fa-trash-alt"></i></a>
                  </td>
                </tr>
              <?php endwhile; ?>
            <?php else: ?>
