...
if (!empty($search)) {
    $search_param = "%$search%";
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM dealer WHERE name LIKE ? OR email LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT * FROM dealer WHERE name LIKE ? OR email LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?");
    $stmt->bind_param("ssii", $search_param, $search_param, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // Count total technicians
    $count_result = $conn->query("SELECT COUNT(*) AS total FROM dealer");
    $total_row = $count_result->fetch_assoc();
    $total_records = $total_row['total'];

    // Get technicians with pagination
    $sql = "SELECT * FROM dealer ORDER BY id DESC LIMIT $limit OFFSET $offset";
  ...
  <div class="container">
    <div class="sidebar">
        ...
        <div class="submenu" id="manageMenu">
            ...
            <a href="manage_dealer.php" class="active"><i class="fas fa-user-cog"></i> Manage Dealer</a>
  ...
  <div class="table-container">
    <table>
      ...
      <tbody>
        <?php if ($result && $result->num_rows > 0): ?>
          <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                <a href="edit_dealer.php?id=<?= $row['id'] ?>" title="Edit"><i class="fas fa-edit"></i></a>
                &nbsp;
        ...
        <?php else: ?>
          <tr>
            <td colspan="7" class="no-data">No Dealer found.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
