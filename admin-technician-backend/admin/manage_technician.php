...
// Prepare count query to get total records for pagination
if (!empty($search)) {
    $search_param = "%$search%";
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM techniciam WHERE name LIKE ? OR email LIKE ?");
    ...
    // Prepare main data query with limit/offset
    $stmt = $conn->prepare("SELECT * FROM techniciam WHERE name LIKE ? OR email LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?");
    $stmt->bind_param("ssii", $search_param, $search_param, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // Count total technicians
    $count_result = $conn->query("SELECT COUNT(*) AS total FROM techniciam");
    $total_row = $count_result->fetch_assoc();
    $total_records = $total_row['total'];

    // Get technicians with pagination
    $sql = "SELECT t.*, r.name as role_name 
        FROM techniciam t 
  ...
  <div class="container">
    <div class="sidebar">
        ...
        <div class="submenu" id="manageMenu">
            ...
            <a href="manage_technician.php" class="active"><i class="fas fa-user-cog"></i> Manage Technician</a>
    ...
    <form method="GET" class="search-form">
      <input type="text" name="search" placeholder="Search by product or issue" value="<?= htmlspecialchars($search) ?>" />
        ...
        <?php if ($result && $result->num_rows > 0): ?>
          <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td>
                  <?php if (!empty($row['id_proof'])): ?>
                      <a href="../<?= htmlspecialchars($row['id_proof']) ?>" target="_blank">View</a>
                  <?php else: ?>
                      -
                  <?php endif; ?>
              </td>
              <td>
                  <?php if (!empty($row['agreement'])): ?>
                      <a href="../<?= htmlspecialchars($row['agreement']) ?>" target="_blank">View</a>
                  <?php else: ?>
                      -
                  <?php endif; ?>
              </td>
            ...
            ?>
              ...
              <td>
                <a href="edit_technician.php?id=<?= $row['id'] ?>" title="Edit"><i class="fas fa-edit"></i></a>
                &nbsp;
                <a href="delete_technician.php?id=<?= $row['id'] ?>" title="Delete" onclick="return confirm('Are you sure you want to delete this technician?')"><i class="fas fa-trash-alt"></i></a>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
...
document.querySelectorAll('.role-dropdown').forEach(dropdown => {
  dropdown.addEventListener('change', function() {
    ...
    fetch('update_role.php', {
      ...
      body: `id=${technicianId}&role_id=${roleId}`
    })
    .then(res => res.text())
    .then(response => {
