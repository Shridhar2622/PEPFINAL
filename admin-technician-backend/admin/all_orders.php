...
if (!empty($search)) {
    ...
    $stmt_count = $conn->prepare("SELECT COUNT(*) AS total FROM orders 
        JOIN users ON orders.customer_id = users.id 
        WHERE 
            orders.product_name LIKE ? OR 
            orders.problem_description LIKE ? OR 
            users.name LIKE ? OR 
            users.phone LIKE ? OR 
            users.address LIKE ? OR 
            orders.customer_note LIKE ? OR 
            orders.technician_note LIKE ?");
    ...
    $stmt = $conn->prepare("SELECT 
                            ...
                            FROM orders 
                            JOIN users ON orders.customer_id = users.id 
                            LEFT JOIN amount_reasons ON orders.extra_reason_id = amount_reasons.id
                            WHERE 
                                orders.product_name LIKE ? OR 
                                orders.problem_description LIKE ? OR 
                                users.name LIKE ? OR 
    ...
    // No search term
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM orders")->fetch_assoc()['total'];
...
<div class="container">
    <div class="sidebar">
        ...
        <a href="all_orders.php" class="active"><i class="fas fa-clipboard-list"></i> All Orders</a>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    if ($row['status'] == 'assigned' || $row['status'] == 'done') {
                                        ...
                                        ?>
                                        <form method="POST" action="assign_order.php" class="assign-form">
                                ...
                                <td>
                                    ...
                                    <?php elseif ($row['status'] === 'assigned'): ?>
                                        <form method="POST" action="remove_assignment.php" onsubmit="return confirm('Remove technician assignment and set back to pending?');">
                                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>" />
