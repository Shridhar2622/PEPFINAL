...
if (!empty($search)) {
    ...
    // Count query with dealer join
    $stmt_count = $conn->prepare("
        SELECT COUNT(*) AS total 
        FROM pre_orders 
        JOIN dealer ON pre_orders.dealer_id = dealer.id
        WHERE 
            pre_orders.product_name LIKE ? OR 
            pre_orders.problem_description LIKE ? OR 
            pre_orders.customer_name LIKE ? OR 
            pre_orders.phone LIKE ? OR 
            pre_orders.address LIKE ? OR 
            pre_orders.customer_note LIKE ? OR 
            pre_orders.technician_note LIKE ? OR
            dealer.name LIKE ? OR
            dealer.phone LIKE ?
    ");
    $stmt_count->bind_param("sssssssss", $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param);
    $stmt_count->execute();
    $total_records = $stmt_count->get_result()->fetch_assoc()['total'];
    $stmt_count->close();

    // Fetch matching records with dealer info
    $stmt = $conn->prepare("
        SELECT 
            pre_orders.*, 
        ...
        FROM pre_orders
        JOIN dealer ON pre_orders.dealer_id = dealer.id
        LEFT JOIN amount_reasons ON pre_orders.extra_reason_id = amount_reasons.id
        WHERE 
            pre_orders.product_name LIKE ? OR 
            pre_orders.problem_description LIKE ? OR 
            pre_orders.customer_name LIKE ? OR 
        ...
        LIMIT ? OFFSET ?
    ");
    $stmt->bind_param("ssssssssssi", $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $search_param, $limit, $offset);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    // No search term → Fetch all
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM pre_orders")->fetch_assoc()['total'];
...
<div class="container">
    <div class="sidebar">
        ...
        <a href="pre_orders.php" class="active"><i class="fas fa-clipboard-list"></i> Pre Orders</a>
                    ...
                    <?php if ($result && $result->num_rows > 0): ?>
                        <?php $i = $offset + 1; while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars($row['dealer_phone']) ?></td>
                                ...
                                <td>
                                    ...
                                    if ($row['status'] == 'assigned' || $row['status'] == 'done') {
                                        ...
                                        ?>
                                        <form method="POST" action="pre_assign_order.php" class="assign-form">
                                ...
                                <td>
                                    <?php if ($row['status'] === 'pending'): ?>
                                        <form method="POST" action="pre_cancel_order.php" onsubmit="return confirm('Are you sure you want to cancel this order?');">
                                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>" />
                                            <button type="submit" class="cancel-btn">Cancel</button>
                                        </form>
                                    <?php elseif ($row['status'] === 'assigned'): ?>
                                        <form method="POST" action="pre_remove_assignment.php" onsubmit="return confirm('Remove technician assignment and set back to pending?');">
                                            <input type="hidden" name="order_id" value="<?= $row['id'] ?>" />
...
<td>
    <?php
    // Technician name fetch karna based on technician_id
$tech_name = '';
foreach ($technicians as $tech) {
    if ($tech['id'] == $row['technician_id']) {
        $tech_name = $tech['name']; // Sirf name, role hata diya
        break;
    }
}
    ?>

    <!-- ✅ Service Registration SMS Copy -->
    <button class="sms-btn" onclick="copyServiceSMS(
        '<?= htmlspecialchars($row['customer_name']) ?>',
        '<?= htmlspecialchars($row['product_name']) ?>',
        '<?= htmlspecialchars($tech_name) ?>',
        '<?= htmlspecialchars($row['preferred_date']) ?>',
        '<?= htmlspecialchars($row['initial_amount']) ?>',
        '<?= htmlspecialchars($row['customer_note'] ?? '-') ?>'
    )">Copy Service SMS</button>
    
    
    <!-- ✅ Happy PIN SMS Copy -->
    <button class="sms-btn" onclick="copyHappyPIN(
        '<?= htmlspecialchars($row['customer_name']) ?>',
        '<?= htmlspecialchars($row['securitypin']) ?>'
    )">Copy Happy PIN</button>
</td>

<script>
function copyServiceSMS(name, product, tech, date, amount, note) {
    const message = `Dear ${name},
Your ${product} service request has been successfully registered.
Our technician ${tech} will visit your address on ${date}.
Note: ${note}
Initial Service Charge: ₹${amount}

Want to track your service or book again easily? Install Reservice App from Play Store.

Thank you,
Reservice Team`;
    
    navigator.clipboard.writeText(message).then(() => {
        alert("Service SMS copied to clipboard!");
    });
}

function copyHappyPIN(name, pin) {
    const message = `Dear ${name},
Your service is almost complete. Please share the Happy PIN only after you're satisfied with the work.
This PIN confirms that the job is done properly.

Your Happy PIN: ${pin}

Thank you for choosing Reservice!`;

    navigator.clipboard.writeText(message).then(() => {
        alert("Happy PIN SMS copied to clipboard!");
    });
}
</script>
                            </tr>
